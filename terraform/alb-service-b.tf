# -------------------------------------------------------
# ALB for Service B - Application Load Balancer
# Creates an internet-facing ALB that distributes HTTP traffic
# to Service B ECS tasks. Includes a second "green" target group
...
# Access via: http://<alb-dns-name> → forwards to port 3000.
# -------------------------------------------------------

# Internet-facing ALB spanning both public subnets for HA.
resource "aws_lb" "service_b" {
  name               = "${var.project_name}-${var.environment}-alb-b"
  internal           = false
...
resource "aws_lb_listener" "service_b_test" {
  load_balancer_arn = aws_lb.service_b.arn
  port              = 8080
  protocol          = "HTTP"
  ...
  default_action {
    type             = "forward"
    target_group_arn = aws_lb_target_group.service_b_green.arn
  }
...
resource "aws_lb_target_group" "service_b_green" {
  name        = "${var.project_name}-${var.environment}-tg-b-green"
  port        = var.service_b_port
  protocol    = "HTTP"
  vpc_id      = aws_vpc.main.id
  target_type = "ip"
